...
public partial class StatItem : Node
{
    ...
    public StatItem()
    {
        ...
        if (mode < 0.2f)
        {
            percentageValue = (float)
                GD.RandRange(CalcPercentageValue(newrarity)[0], CalcPercentageValue(newrarity)[1]);
        ...
        else
        {
            rawValue = CalcStat(
                stat,
                (float)GD.RandRange(CalcRawValue(newrarity)[0], CalcRawValue(newrarity)[1])
            );
            if (rawValue < 1)
                QueueFree();
        }
    ...
    {
        if (rarity < common)
            return commonRawValues;
        if (rarity < common + uncommon)
            return uncommonRawValues;
        if (rarity < common + uncommon + rare)
            return rareRawValues;
    ...
    {
        if (rarity < common)
            return commonPercentageValues;
        if (rarity < common + uncommon)
            return uncommonPercentageValues;
        if (rarity < common + uncommon + rare)
            return rarePercentageValues;
    ...
    {
        if (stat.GetType() == typeof(int))
            this.stat = stat;
        StatManager sm = p.GetNode<StatManager>("StatManager");
        GD.Print("setting stat:" + stat);
        if (!mode)
